...
namespace Mesen.ViewModels
{
	public class MainMenuViewModel : ViewModelBase
	{
		...
		// 状态绑定：软盘镜像加载状态，绑定到 XAML 的 TextBlock
		[Reactive] public string FloppyStatus { get; set; } = "软盘已弹出！";

		public void SetFloppyStatus(string status)
		{
			if(Dispatcher.UIThread.CheckAccess()) {
				FloppyStatus = status;
			} else {
				Dispatcher.UIThread.Post(() => FloppyStatus = status);
			}
		}
				...
				new MainMenuAction() {
					...
					SubActions = new() {
						new MainMenuAction() {
							...
							CustomText = "磁盘管理",
								OnClick = () => {
									// 只允许单个磁盘管理窗口存在：如果已存在则激活并返回；否则创建并按主窗口定位
									var mainWnd = Mesen.Utilities.ApplicationHelper.GetActiveOrMainWindow();
									// 先尝试获取已存在的窗口
									var existing = Mesen.Utilities.ApplicationHelper.GetExistingWindow<Mesen.Windows.DiskManagementWindow>();
									if(existing != null) {
									...
									if(mainWnd != null) {
										try {
											...
											// 固定 DIP 宽度以便稳定定位
											if(double.IsNaN(dlg.Width) || dlg.Width <= 0) {
												dlg.Width = 360; // DIP
											}
											try {
												...
												dlg.Opened += openedHandler;
												// 非模态显示并以主窗口为所有者，以便在 Opened 处理器里可以定位
												dlg.Show(mainWnd);
												return;
											} catch {
												// 若定位逻辑失败，则回退到通用创建器
						...
						new MainMenuAction() {
							...
							CustomText = "装载磁盘",
							OnClick = async () => {
								// 与主菜单中的装载逻辑保持一致：弹出文件选择对话框并调用核心加载接口
								string? filePath = await FileDialogHelper.OpenFile(null, wnd, "ima", "img");
								if(!string.IsNullOrEmpty(filePath) && File.Exists(filePath)) {
									try {
										int state = EmuApi.FloppyLoadDiskImage(filePath);
									} catch {
										// ignore errors from core
									}
									// 更新状态显示
									SetFloppyStatus(Path.GetFileName(filePath));
								}
							}
						},
						new MainMenuAction() {
							...
							CustomText = "弹出磁盘",
							OnClick = () => {
								// 与主菜单中的弹出逻辑一致：调用核心弹出并更新状态
								try {
									EmuApi.FloppyEject();
								} catch { }
								SetFloppyStatus("软盘已弹出！");
							}
						},
					}
				},

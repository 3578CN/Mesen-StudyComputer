		...
		public static async Task<UpdatePromptViewModel?> GetUpdateInformation(bool silent)
		{
			...
			try {
				using(var client = new HttpClient()) {
					// 自动更新查询地址。
					string updateData = await client.GetStringAsync("https://www.3578.cn/mesen/Services/v1/latestversion.json");
					...
					if(
						...
						updateInfo.Files.Where(f => f.DownloadUrl == null || (!f.DownloadUrl.StartsWith("https://www.3578.cn/") && !f.DownloadUrl.StartsWith("https://github.com/SourMesen/"))).Count() > 0
					) {
						return null;
					}
				}

...
namespace Mesen.Windows
{
	public class MainWindow : MesenWindow
	{
		...
		{
			// Support dropping of .ima floppy images anywhere in the main window.
			var files = e.Data.GetFiles();
			if(files == null || !files.Any()) {
				return;
			}
			try {
				// Build list of local paths
				var paths = files.Select(f => f.Path.LocalPath).Where(p => !string.IsNullOrEmpty(p)).ToArray();
				if(paths.Length == 0) {
					return;
				}
				// If any dropped file is an .ima or .img, treat it as a floppy image instead of a ROM
				string? filePath = paths.FirstOrDefault(p => {
					string ext = Path.GetExtension(p);
					return ext.Equals(".ima", StringComparison.OrdinalIgnoreCase) || ext.Equals(".img", StringComparison.OrdinalIgnoreCase);
				});
				if(filePath != null && File.Exists(filePath)) {
					// Save path for core and update UI status
					// 拖放进来的装载软盘。
					EmuApi.FloppyLoadDiskImage(filePath);
					Dispatcher.UIThread.Post(() => {
						if(_mainMenu?.DataContext is Mesen.ViewModels.MainMenuViewModel mmvm) {
							mmvm.SetFloppyStatus(Path.GetFileName(filePath));
						}
					});
					Activate();
					return;
				...
				// Fallback: load first file as ROM

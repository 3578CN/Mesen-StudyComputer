...
public:
	...
	// 软盘控制器访问器。
	// 现在返回由 InteropDLL 中管理的全局 _fdc 实例（如果存在），
	// 如果未创建则返回 nullptr。
	FloppyDriveController* GetFdc();
	...
	// 切换到 UM6576 PPU 实例。
	// 参数 isUm6576：是否切换到 UM6576 PPU 实例（如果 true：切换到 UM6576 PPU 实例，如果 false：切换到默认 PPU 实例）。
	void SetPpuUm6576(bool isUm6576);

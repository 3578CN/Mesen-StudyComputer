...
static const int32_t lpc_k1_tab[] =
{
...
static const int32_t lpc_k2_tab[] =
{
...
static const int32_t lpc_k3_tab[] =
{
...
static const int32_t lpc_k4_tab[] =
{
...
// bits <= 8
static short get_nbits(LPC* s, short bits)
{
	unsigned short data;
...
static int lpc_get_frame(LPC* s, LPC_FRAME* dst, LPC_FRAME* ref)
{
	int pitch_i;
	int energy_i;
	int repeat;
...
static void lpc_set_interp_flag(LPC* s)
{
	s->need_interp = 1;
...
static void lpc_init(LPC* s)
{
	// clear reset
	memset(s, 0, sizeof(LPC));

	s->random_seed = (short)0xFFFF;
}

static int lpc_preload(LPC* s)
{
	int magic;

	magic = get_nbits(s, 8);

	if (LPC_STD_VARIANT_SB2K == s->variant) {
		// 0x50 = bit rev 0x0A
		if (0x50 != magic)
			return 0;
	} else {
...
static short lpc_reload_pitch(LPC* s)
{
	short curr_pitch;
...
static void lpc_do_filter(LPC* s)
{
	int i;
	int sample;
	short* x;
...
static short* lpc_run(LPC* s, short* pcm_out, int* eos)
{
	int i;
	int excit;
	...
	if (lpc_get_frame(s, &s->frame_next, &s->frame_prev)) {
		...
		// need restart
		if (s->variant == LPC_STD_VARIANT_SB2K) {
...
void* lpc_d6_new(int (*feed)(void*, unsigned char*), void* host, int variant)
{
	LPC* lpc;

	lpc = (LPC*)malloc(sizeof(LPC));
	if (!lpc) return 0;
...
int lpc_d6_reset(void* lpc)
{
	...
	s->feed = (int (*)(void*, unsigned char*))feed;
	s->host = host;
	s->variant = variant;

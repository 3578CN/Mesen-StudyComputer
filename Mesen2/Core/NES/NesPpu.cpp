...
// 为 Sb2kPpu 显式生成 NesPpu 成员函数的符号（解决链接器未定义符号）
// template void NesPpu<Sb2kPpu>::SetBusAddress(uint16_t addr);
template uint8_t NesPpu<Sb2kPpu>::ReadVram(uint16_t addr, MemoryOperationType type);
template void NesPpu<Sb2kPpu>::WriteVram(uint16_t addr, uint8_t value);
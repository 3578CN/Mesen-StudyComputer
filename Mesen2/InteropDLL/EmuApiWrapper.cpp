...
// 全局唯一的 FloppyDriveController 实例，由此文件负责所有实例化与管理
unique_ptr<FloppyDriveController> _fdc;
...
extern "C" {
	...
	// Floppy drive APIs
	DllExport int __stdcall Floppy_LoadDiskImage(char* filename)
	{
	...
	DllExport int __stdcall Floppy_Eject()
	{
		if(_fdc) {
			return _fdc->Eject();
		}
